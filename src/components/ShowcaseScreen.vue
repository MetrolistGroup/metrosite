...
<template>
  <div class="screen" :class="`screen--${device}`">
    <img v-if="source && !failed" class="screen__capture" :src="source" :alt="`Metrolist ${screenName} on ${platformName}`" loading="lazy" decoding="async" @error="failed = true" />
...
.screen__capture { width: 100%; height: 100%; object-fit: contain; }
